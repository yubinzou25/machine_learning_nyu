...
# Student side autogradingx was added by Brad Miller, Nick Hay, and
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.maxPoints = 20 * sum([len(t) for t in [self.scoreThresholds, self.nonTimeoutThresholds, self.winsThresholds]])
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for passed, points, value, minimum, thresholds, name in results:
            ...
            if not passed:
                ...
                self.addMessage("%s %s (%s of %s points)" % (value, name, points, self.maxPoints))
            ...
            if minimum != None:
                ...
                for idx, threshold in enumerate(thresholds):
                    self.addMessage("    >= %s:  %s points" % (threshold, (idx+1) * self.maxPoints / len(thresholds)))
        ...
        if any([not passed for passed, _, _, _, _, _ in results]):
            totalPoints = 0
        print(totalPoints, self.maxPoints)
