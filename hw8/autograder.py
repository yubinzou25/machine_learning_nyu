...
class Tracker(object):
    def __init__(self, questions, maxes, prereqs, mute_output, gs_output=False):
...
""")
    ...
    def produce_gradescope_output(self):
        print("producing gradescope output...")
        out_dct = {}
...
def main():
    ...
    tracker = Tracker(questions, maxes, PREREQS, options.mute_output, gs_output=options.gs_output)
    for q in questions:
...
def check_perceptron(tracker):
    ...
    tracker.add_points(8) # Partial credit for passing sanity checks
